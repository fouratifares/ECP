...
class Function:
    ...
    def load_dataset(self) -> pd.DataFrame:
        """Load the dataset from the UCI repository and handle missing values."""
    ...
    def preprocess_data(self) -> tuple:
        ...
        # Drop rows with NaN values and remove the 'car_name' column (non-numeric)
        data = self.data.dropna().drop(columns=['car_name'])

        # Extract features (X) and target (y)
        X = data.iloc[:, 1:].values  # Features (excluding mpg which is the target)
        y = data.iloc[:, 0].values  # Target (mpg)
